...
void Scamp5cApp::host_callback_events(){
    ...
    int dim = s5cHost->GetDataDim(0);

    while(events_frame_series.size()>=coordinates_trail_length){
        delete events_frame_series.front();
        events_frame_series.pop_front();
    }

    EventsCount = 0;

    auto v = new events_frame(dim);
    ...
    events_frame_series.push_back(v);
...
void Scamp5cApp::host_callback_target(){
    ...
    while(target_series.size()>=coordinates_trail_length){
        delete target_series.front();
        target_series.pop_front();
    }
